#!/usr/bin/env python3
# batch_backtest.py  (with optional RSI entry filter)
...
class Config:
    ...
    # NEW: RSI filter
    rsi_len: int = 14
    rsi_long: float | None = None   # e.g., 35 to require oversold for longs
    rsi_short: float | None = None  # e.g., 65 to require overbought for shorts
...
def rolling_regression_channel(price: pd.Series, window: int, k: float
    ...
    for i in range(window, n):
        ...
        band = k * sigma
        mid[i]=y_hat_now; up[i]=y_hat_now+band; lo[i]=y_hat_now-band; slope[i]=beta1; bw[i]=band
    idx = price.index
...
# ---------------- Signals (z-threshold, cross, cooldown, min-hold, RSI) ----------------
def generate_signals(df: pd.DataFrame, cfg: Config) -> pd.DataFrame:
    ...
    # RSI once
    rsi_series = rsi(price, cfg.rsi_len).values
    ...
    for i in range(1, len(out)):
        ...
        # Entries (flat, not cooling) with RSI filter
        if not in_long and not in_short and cooldown == 0:
            r_ok_long = True if cfg.rsi_long is None else (rsi_series[i-1] <= cfg.rsi_long)
            r_ok_short = True if cfg.rsi_short is None else (rsi_series[i-1] >= cfg.rsi_short)
...
def summarize(perf: pd.DataFrame, time_col: str) -> pd.Series:
    ...
    return pd.Series(dict(
        cum_return=cumret, sharpe=sharpe, max_drawdown=max_dd,
        volatility=float(vol*np.sqrt(n_per_year)), avg_bar_return=avg,
        bars_per_year=float(n_per_year), bars=int(len(perf)), trades=trades
    ))

# ---------------- Batch runner ----------------
def parse_args():
    ap = argparse.ArgumentParser(description="Batch backtest regression-channel strategy with RSI filter.")
    ...
    ap.add_argument("--resample", default=None, help='Pandas freq like "1T" or "5T"')
...
def main():
    ...
    if not paths:
        print("No CSVs found.", file=sys.stderr); return 2

    summaries = []
    for pth in paths:
        try:
            cfg = Config(
                csv_path=pth, price_col=args.price_col, time_col=args.time_col,
                window=args.window, k=args.k, stop_k=args.stop_k,
                allow_short=args.allow_short, fee_bps=args.fee_bps,
                slippage_bps=args.slippage_bps, tz_localize=args.tz_localize,
                resample=args.resample, z_entry=args.z_entry,
                slope_min=args.slope_min, min_hold=args.min_hold,
                cooldown=args.cooldown, rsi_len=args.rsi_len,
                rsi_long=args.rsi_long, rsi_short=args.rsi_short
            )
            df = load_data(cfg)
    ...
    if summaries:
        ...
        cols = ["file","symbol_hint","cum_return","sharpe","max_drawdown","volatility","avg_bar_return","bars_per_year","bars","trades"]
